...
public class TrayManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (currentTargetTray == null || !currentTargetTray.isInTutorialArea)
        {
    ...
    {
        Tray targetTray = GetRandomVisibleTrayWithEmptySlot();
        if (targetTray == null)
            return;

        Tray sourceTray = GetRandomVisibleTrayWithItemExcept(targetTray);
        if (sourceTray == null)
            return;

        DragItem item = GetAnyItemFromTray(sourceTray);
        if (item == null)
            return;
