...
public class Tray : MonoBehaviour
{
    ...
    public void CheckMatch()
    {
        List<DragItem> itemsInSlot = new List<DragItem>();
        ...
        foreach (var slot in slots)
        {
            ...
            itemsInSlot.Add(slot.currentItem);
        }

        if (itemsInSlot.Count < 3) return;

        var groups = itemsInSlot.GroupBy(i =>
        {
            var sr = i.GetComponent<SpriteRenderer>();
            return sr != null && sr.sprite != null
                ? sr.sprite.name
        ...
        foreach (var g in groups)
        {
            ...
            HandleMatch(g.Take(3).ToList());
            return;
        }
    }
    void HandleMatch(List<DragItem> matchedItems)
    {
        GameManager.instance.countOrder++;
        count = GameManager.instance.countOrder;
        ...
        GameEvent.OnTrayEmpty?.Invoke();
        GameEvent.OnRemoveIce?.Invoke();
    }
    ...
    void SpawnDisk(List<DragItem> items, Vector3 centerPos)
    {
        ...
        GameObject diskObj = Instantiate(diskPrefab, spawnPos, Quaternion.identity);
        ...
        foreach (var item in items)
        {
            ...
            Sequence seq = DOTween.Sequence();
            var jumpTween = it.transform.DOJump(targetPos, jumpPower, jumpCount, itemToDiskTime).SetEase(Ease.InOutQuad);
            var scaleTween = it.transform.DOScale(targetScale, itemToDiskTime).SetEase(Ease.OutQuad);
